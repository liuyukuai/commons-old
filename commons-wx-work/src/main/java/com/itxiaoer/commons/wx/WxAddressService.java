...
public class WxAddressService {
    ...
    private LoadingCache<String, String> JsTokenCache
            = Caffeine.newBuilder()
            //设置写缓存后1个小时过期
            .expireAfterWrite(1, TimeUnit.HOURS)
            //设置缓存容器的初始容量为10
            ...
            .build((key) -> {
                ResponseEntity<String> response = restTemplate.getForEntity(String.format(WxConstants.WX_JS_TOKEN_URL, getToken()), String.class);
                if (log.isDebugEnabled()) {
                    log.debug("oad wx token response = {} ", response);
                }
                ...
                String body = response.getBody();
                if (StringUtils.isNotBlank(body)) {
                    return JsonUtil.toBean(body, Token.class).map(Token::getTicket).orElseGet(() -> {
                        log.warn("load wx token  error , {} ", response);
                        return "";
                    });
                }
    ...
    public String getJsToken() {
        try {
            return JsTokenCache.get("jsToken");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return null;
        }
